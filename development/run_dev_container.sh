...
sudo docker build -t microservice_template_dev.image . # build the image and names it 'microservice_template_dev.image'
...
docker exec -it microservice_template_dev.container bash -c 'npm run dev'  # connect to a cli inside the container
#
